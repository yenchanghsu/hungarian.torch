...
static int solver(lua_State *L)
{
  ...
  h = THIntTensor_size(tensor,0);
  w = THIntTensor_size(tensor,1);
  THIntTensor *tensorc = THIntTensor_newContiguous(tensor);

  int** m = array_to_matrix((int*)THIntTensor_data(tensorc),h,w);
  int matrix_size = hungarian_init(&p, m , w,h, HUNGARIAN_MODE_MINIMIZE_COST) ;
  ...
  int numElements = p.num_rows;
  int *mydata = (int*)malloc(sizeof(int) * numElements);
  for(i=0;i<p.num_rows;i++)
  {
    for(j=0;j<p.num_cols;j++)
    {
  ...
  THIntStorage *mystorage =  THIntStorage_newWithData(mydata, numElements);
  THIntTensor* mytensor = THIntTensor_newWithStorage1d(mystorage, 0, numElements, 1);
  luaT_pushudata(L, (void *)mytensor, "torch.IntTensor");
...
static const luaL_Reg hungarianlib[] = {
  {"solver",   solver},
  {NULL, NULL}
};

int luaopen_libhungarian (lua_State *L) {
  luaL_openlib(L, "libhungarian", hungarianlib, 0);
  return 1;
}
